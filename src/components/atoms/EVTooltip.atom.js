...
export default function CustomTooltip({ children }) {
  ...
  const getTooltipLabel = (label) => {
    return (
      <span style={{backgroundColor: 'rgba(0, 0, 0, 0.8)', color: 'white', padding: '.5rem 1rem', borderRadius: '.25rem', fontSize: '.875rem'}}>
        {label}
      </span>
    )
  }
  ...
  return (
    <div style={{ height: "100%", width: "100%" }}>
      ...
      <TooltipProvider openTooltip={openTooltip} closeTooltip={closeTooltip} getTooltipLabel={getTooltipLabel}>
        {children}
      </TooltipProvider>
    </div>
  );
...
function TooltipProvider({ children, openTooltip, closeTooltip, getTooltipLabel }) {
  ...
  }, [openTooltip, closeTooltip, getTooltipLabel]);
